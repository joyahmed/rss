...
const POSTS_DIRECTORY = path.join(process.cwd(), 'components_mdx', 'posts');
...
export const GET = async () => {
  ...
  fileNames.forEach((fileName) => {
    ...
    if (!data.title || !data.description || !data.date || !data.image) {
      throw new Error(`Markdown file ${fileName} is missing required frontmatter fields.`);
    }
